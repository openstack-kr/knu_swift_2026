...
class SwiftRecon(object):
    ...
    def container_sync_check(self, hosts):
        """
        Obtain and print container-sync statistics
        ...
        recon = Scout("container-sync", self.verbose,
                      self.suppress_errors, self.timeout)
        print("[%s] Checking on container-sync" % self._ptime())
        ...
        for url, response, status, ts_start, ts_end in self.pool.imap(
            ...
            if status == 200:
                ...
                containers = response.get('container_sync_containers') or {}
                if isinstance(containers, dict):
                    scanned_containers += len(containers)
                    for item in containers.values():
                        if not isinstance(item, dict):
                            continue
                        new_backlog_rows += as_int(item.get(
                            'new_backlog_rows'))
                        retry_backlog_rows += as_int(item.get(
                            'retry_backlog_rows'))
                        if item.get('last_status') == 'failure':
                            failed_containers += 1
                        if item.get('time_exhausted'):
                            time_exhausted_containers += 1
        ...
        for k in stats:
            if stats[k]:
                ...
                print("[%s] - No hosts returned valid data." % k)
        self._calculate_least_and_most_recent(last_stats)
        print("Container sync recon containers: %d, failed: %d, "
              "time_exhausted: %d, new_backlog_rows: %d, "
              "retry_backlog_rows: %d" % (
                  scanned_containers, failed_containers,
                  time_exhausted_containers, new_backlog_rows,
                  retry_backlog_rows))
        print("=" * 79)
    ...
    def main(self):
        ...
        args.add_option('--container-sync', action="store_true",
                        dest="container_sync",
                        help="Get container-sync stats")
        ...
        for server_type in server_types:
            ...
            if options.all:
                if self.server_type == 'object':
                    ...
                    self.container_sync_check(hosts)
                ...
                if options.sharding:
                    if self.server_type == 'container':
                        ...
                        print("Error: Can't check sharding on non container "
                              "servers.")
                        print("=" * 79)
                if options.container_sync:
                    if self.server_type == 'container':
                        self.container_sync_check(hosts)
                    else:
                        print("Error: Can't check container-sync on non "
                              "container servers.")
                        print("=" * 79)
