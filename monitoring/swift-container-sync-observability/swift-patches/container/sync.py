...
class ContainerSync(Daemon):
    ...
    def __init__(self, conf, container_ring=None, logger=None):
        ...
        self.rcache = os.path.join(
            self.recon_cache_path, RECON_CONTAINER_FILE)
    ...
    def run_forever(self, *args, **kwargs):
        ...
        while True:
            ...
            elapsed = time() - begin
            dump_recon_cache({
                'container_sync_sweep': elapsed,
                'container_sync_last': time(),
    ...
    def log_sync_event(self, event):
        self.logger.info('container-sync-event %s',
                         json.dumps(event, sort_keys=True))

    def remote_container_path(self, sync_to):
        return urlparse(sync_to).path

    def _status_class(self, status_code):
        if status_code is None:
            return ''
        try:
            return '%dxx' % (int(status_code) // 100)
        except (TypeError, ValueError):
            return ''

    def log_object_sync_event(self, row, info, sync_to, method, outcome,
                              reason, start_time, status_code=None):
        end_time = time()
        source_path = '/v1/%s/%s/%s' % (
        ...
        event = {
            ...
            'bytes': row.get('size', 0),
            'source_path': source_path,
            'remote_container_path': self.remote_container_path(sync_to),
            'duration_ms': (end_time - start_time) * 1000,
            'timestamp': end_time,
        }
        if status_code is not None:
            event['status_code'] = int(status_code)
            event['status_class'] = self._status_class(status_code)
        self.log_sync_event(event)
    ...
    def report(self):
        ...
        dump_recon_cache({
            'container_sync_stats': {
                'syncs': self.container_syncs,
                'deletes': self.container_deletes,
                'puts': self.container_puts,
                'skips': self.container_skips,
                'failures': self.container_failures,
            },
        }, self.rcache, self.logger)
        self.reported = time()
        self.container_syncs = 0
    ...
    def container_report(self, start, end, sync_point1, sync_point2, info,
        ...
        self.logger.info('Container sync report: %(container)s, '
                         ...
                         {'container': '%s/%s' % (info['account'],
                          ...
                          'point2': sync_point2,
                          'total': max_row})
        progress_point = max(sync_point1, sync_point2 or 0)
        if max_row:
            progress_point = max(0, min(progress_point, max_row))
            progress_percent = (float(progress_point) / max_row) * 100
        else:
            progress_percent = 0
        self.log_sync_event({
            'event_type': 'container_sync_report',
            'account': info['account'],
            'container': info['container'],
    ...
    def container_sync(self, path):
        ...
        try:
            ...
            if broker.metadata.get(SYSMETA_VERSIONS_CONT):
                ...
                self.log_sync_event({
                    'event_type': 'container_sync_state',
                    'account': info['account'],
                    'container': info['container'],
                    'state': 'skipped',
                    'reason': 'object_versioning_configured',
                    'timestamp': time(),
                })
                return
            if not broker.is_deleted():
                ...
                if not sync_to or not user_key:
                    ...
                    self.log_sync_event({
                        ...
                        'container': info['container'],
                        'state': 'skipped',
                        'reason': 'missing_sync_to_or_user_key',
                        'timestamp': time(),
                    })
                    return
        ...
        try:
            ...
            if row['deleted']:
                ...
                if self._object_in_remote_container(row['name'],
                                                    ...
                                                    realm_key, ts_meta):
                    self.log_object_sync_event(
                        row, info, sync_to, 'HEAD', 'skipped',
                        'remote_current', start_time)
                    return True
                exc = None
                ...
                if headers.get(SYSMETA_VERSIONS_SYMLINK):
                    self.logger.info(
                        'Skipping versioning symlink %s/%s/%s ' % (
                            ...
                            row['name']))
                    self.log_object_sync_event(
                        row, info, sync_to, 'PUT', 'skipped',
                        'versioning_symlink', start_time)
                    return True
            ...
            if err.http_status == HTTP_UNAUTHORIZED:
                reason = 'unauthorized'
                self.logger.info(
                    'Unauth %(sync_from)r => %(remote_container_path)r',
                    {'sync_from': '%s/%s' %
                        (quote(info['account']), quote(info['container'])),
                ...
                reason = 'not_found'
                self.logger.info(
                    'Not found %(sync_from)r => %(remote_container_path)r '
                    '- object %(obj_name)r',
                    {'sync_from': '%s/%s' %
                        (quote(info['account']), quote(info['container'])),
                     'remote_container_path': remote_container_path,
            ...
            self.logger.increment('failures')
            self.log_object_sync_event(
                row, info, sync_to, 'DELETE' if row.get('deleted') else 'PUT',
                'failure', reason, start_time, status_code=err.http_status)
            return False
        except (Exception, Timeout):
            self.logger.exception(
                'ERROR Syncing %(db_file)s %(row)s',
