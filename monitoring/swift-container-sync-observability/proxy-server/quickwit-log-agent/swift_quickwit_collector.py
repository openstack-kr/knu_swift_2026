...
def remote_tail_command(args):
    ...
    if args.ssh_proxy_host:
        ...
        proxy_key = args.ssh_proxy_key or args.ssh_key
        if proxy_key:
            proxy_cmd.extend(["-i", proxy_key])
        proxy_cmd.extend(["-W", "%h:%p", args.ssh_proxy_host])
        cmd.extend(["-o", "ProxyCommand=%s" % " ".join(
            shlex.quote(part) for part in proxy_cmd)])
    if args.ssh_key:
...
def main():
    ...
    parser.add_argument("--ssh-proxy-host",
                        help="optional SSH bastion used as ProxyCommand")
    parser.add_argument("--ssh-proxy-key",
                        help="optional SSH key for the bastion; defaults to --ssh-key")
