                ...
                try:
                    ...
                    if sync_point2 < sync_point1:
                        ...
                        for partition in range(len(nodes)):
                            ...
                            partition_state = retry_state[partition_key]
                            partition_point = partition_state['point']
                            if partition_point >= sync_point1:
                                continue

                            # Let the next node take over a stale partition.
                            # 담당 partition 이 오래 멈췄으면 다음 노드가 이어받는다.
                            # Store progress under the partition key, not the executor.
                            # progress 는 실행 노드가 아니라 partition key 에 저장한다.
                            updated_at = float(
                                partition_state.get('updated_at') or 0)
                            stale = updated_at > 0 and \
                                now - updated_at >= \
                                self.retry_takeover_timeout
                            worker = (partition + 1) % len(nodes) \
                                if stale else partition
                            if worker != ordinal:
                                continue

                            retry_fetch_point = partition_point
                            retry_halted = False
                            while time() < stop_at and \
                                ...
                                # Sync only rows owned by the current partition.
                                # 현재 partition 의 row 만 sync 하고,
                                # Advance scan progress across rows from other partitions.
                                # 다른 partition row 는 scan progress 만 전진시킨다.
                                rows_with_partitions = [
